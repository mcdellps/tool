...
usage ()\n\
{\n\
  echo -e \"\n  usage: $0 Oracle_SID \nexample: $0 orcl \n\n Restore and Recover RMAN\nRemove /u01/app/oradata/ORCL/datafile\nopen_mode from v\$database\n\" >&2; exit 1;\n\
}\n\
[ $# -ne $numparms ] && usage\n\
export ORACLE_SID=$1\n\
rman << EOF\n\
...
configure default device type to sbt_tape;\n\
configure channel device type 'sbt_tape' parms 'BLKSIZE=1048576, SBT_LIBRARY=/home/oracle/opt/dpsapps/rmanagent/lib/libddobk.so, ENV=(STORAGE_UNIT=ppdm-su, BACKUP_HOST=ppdm-ip, RMAN_AGENT_HOME=/home/oracle/opt/dpsapps/rmanagent)';\n\
configure controlfile autobackup format for device type sbt_tape to './ppdm-uuid/SS/Full_%d_%I_%u_%p';\n\
restore database;\n\
recover database;\n\
}\n\
EOF\n\
...
void process_option_b()
{
   ...
   create_file("scripts/rman-05.sh", (char *) RMANSCRIPT05);
...
void process_option_j()
{
   ...
   char stor_unit[50], host_ip[30], ppdm_uuid[80];
